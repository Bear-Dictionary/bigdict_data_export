...
def export_dictionary(client: MongoClient, name: str, writer: Callable[[str, str, str, str], None]):
    ...
    fromLang = client.admin.languages.find_one({'_id': dictionary['fromLang']})
    toLang = client.admin.languages.find_one({'_id': dictionary['toLang']})
    # words = client.admin.words.find({'meanings': SON([('dictionary', dictionary['_id'])])})
    words = client.admin.words.find({'meanings.dictionary': dictionary['_id']})

    print('Dictionary:', fromLang['code'], '->', toLang['code'])
    count = 0
    for word in words:
        count += 1
        content = next((x for x in word['meanings'] if x['dictionary'] == dictionary["_id"]), None)
        writer(word['key'], fromLang['code'], toLang['code'], content['htmlString'])
